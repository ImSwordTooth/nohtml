        ...
        return(
            <li id={'border'}>
                ...
                <div className={'content'}>
                    <div className={'items'}>
                        <Slider style={{width:100}} min={0} max={20} onChange={(e)=>changeProp('border',e+'px',0)} value={parseInt(this.state.nocssStyle.border.split(' ')[0])}/><span className={'unit'}>{this.state.nocssStyle.border.split(' ')[0]}</span>
                    </div>
                    <div className={'items flexStart'}>
                        <Select style={{width:150}} defaultValue={this.state.nocssStyle.border.split(' ')[1]} onChange={(e)=>changeProp('border',e,1)}>
                    ...
                    <div className={'items'}>
                        <ColorPicker onChange={(e)=>{changeProp('border',colorRgba(e.color,e.alpha),2)}} defaultColor={this.state.nocssStyle.border.split(' ')[2]} defaultAlpha={new RegExp('(?<=\\()\\S+(?=\\))','g').exec(this.state.nocssStyle.border.split(' ')[2])?Number(new RegExp('(?<=\\()\\S+(?=\\))','g').exec(this.state.nocssStyle.border.split(' ')[2])[0].split(',')[3])*100:100}>
